...
def remove_rows_with_none_elements(blog):
    """
    Removes pairs of lists where either the sentence or the char array has a None value at any index.
    
    Args:
        list_of_lists (list): List of lists, where each sublist contains a sentence list and a char list.
        
    Returns:
        list: A list of pairs (sentence_list, char_list) with rows containing None elements removed.
    """
    cleaned_list1 = []
    cleaned_list2 = []
    cleaned_list = []
    for i in range(len(blog[0])):

        if blog[0][i] is not None and blog[1][i] is not None:
            cleaned_list1.append(blog[0][i])
            cleaned_list2.append(blog[1][i])
        cleaned_list.append(cleaned_list1)
        cleaned_list.append(cleaned_list2)
    return cleaned_list
...
def remove_incorrectly_spelled_words(sentence):
    spell = SpellChecker()
    ...
    return ans, num_incorrect_words